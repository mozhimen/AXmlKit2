package com.mozhimen.xmlk2.layoutk
...
class LayoutKSpinnerForm @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    override fun initAttrs(attrs: AttributeSet?, defStyleAttr: Int) {
        attrs?.let {
            ...
            mIsRequire =
                typedArray.getBoolean(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_isRequired, mIsRequire)
            mRequiredIconPos =
                typedArray.getInteger(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_requiredIconPos, mRequiredIconPos)
            mLabel =
                typedArray.getString(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_label)
            mLabelTextSize =
                typedArray.getDimension(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_labelTextSize, mLabelTextSize)
            mLabelColor =
                typedArray.getColor(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_labelColor, mLabelColor)
            mLabelMarginLeft =
                typedArray.getDimensionPixelSize(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_labelMarginLeft, mLabelMarginLeft)
            mLabelMarginRight =
                typedArray.getDimensionPixelSize(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_labelMarginRight, mLabelMarginRight)
            mLabelWidth =
                typedArray.getDimensionPixelSize(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_labelWidth, mLabelWidth)
            mSpinnerList.addAll(
                generateStringArray(typedArray.getResourceId(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_spinnerEntries, 0))
            )
            mSpinnerItemLayout =
                typedArray.getResourceId(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_spinnerItemLayout, mSpinnerItemLayout)
            mBorderBackground =
                typedArray.getResourceId(R.styleable.LayoutKSpinnerForm_layoutKSpinnerForm_borderBackground, mBorderBackground)
            typedArray.recycle()
        }
    }
